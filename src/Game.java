...
public class Game implements ActionListener, KeyListener
{
		...
		public void run()
		{
			...
			while (true)
			{
				...
				else if (minutes >= 60)
				{
					try
					{
						throw new TimeLimitExceededException("You are taking way too long for this game... Game ending now!");
					}
					catch (TimeLimitExceededException e)
					{
						System.out.println("You are taking way too long for this game... Game ending now!");
						System.exit(1);
					}
				}
	...
	public Game() // The "main" method
	{
		...
		JPanel fields = new JPanel(new GridLayout(3, 2)); // Now fields are used for the following
		...
		f.add(new JScrollPane(outputConsole), BorderLayout.SOUTH); // don't forget JScrollPane()
		f.pack();
		f.setVisible(true);
	...
	private TrayIcon trayIcon = new TrayIcon(Toolkit.getDefaultToolkit().getImage("Suits.png"));
	...
	public void actionPerformed(ActionEvent arg0)
	{
		...
		{
			showMessageDialog(null, "This is a custom game of BlackJack geared toward one human and multiple computer players..."
		...
		else if (arg0.toString().contains("Make Bet"))
		{
			...
			{
				showMessageDialog(null, "The most money you have is $" + viewer.getMoney() + ", but you typed in $" + bet
						+ " as you bet.");
				getPlayerBet();
			}
			else
	...
	public void displayCards() // Works?
	{
		...
		for (int j = 0; j < viewer.getHandList().size(); j++)
		{
			for (int i = 0; i < viewer.getHandList().get(j).getCards().size(); i++)
			{
				...
				if (viewer.getDropped() != null && viewer.getDropped().equals(viewer.getHandList().get(j).getCards().get(i)))
				{
					// If dropped, adds notification and image to the panel
					// System.out.println("A");
					panels.get(panels.size() - 1).setLayout(new BorderLayout());
					panels.get(panels.size() - 1).add(
							new JLabel("You accidentally dropped the card above. Other players know about it..."),
							BorderLayout.SOUTH);
		...
		playerCards.setLayout(new GridLayout(userScreen.height / 250, userScreen.width / 200));
		...
		otherCards.setLayout(new GridLayout(userScreen.height / 250, userScreen.width / 200));
		for (int i = 0; i < panels.size(); i++)
		{
	...
	{
		new Game(); // Just making a new object. Will just like a main method for the object.
	}
}
